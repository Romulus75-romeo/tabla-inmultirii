...
window.LearningModule = {
    ...
    startMiniQuiz() {
        ...
        const showQuestion = () => {
            ...
            // Generare răspunsuri greșite îmbunătățită
            const wrongAnswers = [];
            const minAnswer = Math.max(1, q.answer - 5); // Nu permite răspunsuri sub 1
            const maxAnswer = q.answer + 10;

            while (wrongAnswers.length < 2) {
                const wrong = Math.floor(Math.random() * (maxAnswer - minAnswer + 1)) + minAnswer;
                // Exclude răspunsul corect și evită duplicate
                if (wrong !== q.answer && !wrongAnswers.includes(wrong)) {
